...
const Post = ({ post }) => {
  ...
  //toggle pour editer le content
  const [editToggle, setEditToggle] = useState(false);

  const [editContent, setEditContent] = useState(post.content)

  // verifier dabord si lutilisateur est l' author de larticle part 1
  ...
  const handleEdit = (e) => {
    ...
    // editer avec mongodb est legerement different(on met juste en data les choses a editer et non tout)
    const postData = {
      title: post.title,
      content: editContent,
  ...
  return (
    <div className="post">
      {/* verifier dabord si lutilisateur est l' author de larticle part 2 */}
